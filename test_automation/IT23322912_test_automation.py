...
DEFAULT_EXCEL_CANDIDATES = [
    ...
    str(TESTS_DIR / " IT23322912_Assignment 1 - Test cases.xlsx"),
    str(TESTS_DIR / " Assignment 1 - Test cases.xlsx"),
    str(TESTS_DIR / "IT23322912_Assignment 1 - Test cases.xlsx"),
    str(ROOT_DIR / "IT23322912_Assignment 1 - Test cases.xlsx"),
...
def _resolve_path(p: str | None) -> str | None:
    if not p:
        return None
    # Clean the input path from accidental quotes or spaces
    p = p.strip().strip("'").strip('"')
    
    # 1. Try path exactly as provided
    if Path(p).exists():
        return str(Path(p).resolve())
        
    # 2. Try relative to ROOT and TESTS folders
    for base in [ROOT_DIR, TESTS_DIR]:
        # Try normally
        cand = (base / p).resolve()
        if cand.exists(): return str(cand)
        # Try by stripping leading/trailing spaces from the filename itself
        cand_trimmed = (base / Path(p).name.strip()).resolve()
        if cand_trimmed.exists(): return str(cand_trimmed)
